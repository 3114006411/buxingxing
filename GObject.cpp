...
bool GObject::Collision()
{
	...
	else if (Achive())
	{
		switch (m_cmd)
		{
		case UP:if (m_nRow > 0 && !pStage->mapData[m_nRow-1][m_nArray])
		{
			b = true;
		}
			break;
		case DOWN:if (m_nRow < MAPLENTH - 1 && !pStage->mapData[m_nRow + 1][m_nArray])
		{
			b = true;
		}
			break;
		case LEFT:if (m_nArray > 0 && !pStage->mapData[m_nRow][m_nArray - 1])
		{
			b = true;
		}
			break;
		case RIGHT:if (m_nArray < MAPLENTH - 1 && !pStage->mapData[m_nRow][m_nArray + 1])
		{
			b = true;
		}
			break;
	...
	case UP:if (m_nRow > 0 && !pStage->mapData[m_nRow - 1][m_nArray])
	{
		...
		break;
			...
			break;
	case DOWN: if (m_nRow < MAPLENTH - 1 &&
	...
	case LEFT:  if (m_nArray>0  &&
		...
		break;
	case RIGHT: if (m_nArray < MAPLENTH - 1 &&
		!pStage->mapData[m_nRow][m_nArray + 1]) {
		b = true;
		break;									// "撞墙了"
	}
